...
func resourcePushMirrorRead(d *schema.ResourceData, meta interface{}) (err error) {
	...
	pushMirror, resp, err := client.GetPushMirror(d.Get(pushMirrorOwner).(string), d.Get(pushMirrorRepo).(string), d.Id())
...
func respurcePushMirrorDelete(d *schema.ResourceData, meta interface{}) (err error) {
	...
	client.DeletePushMirror(d.Get(pushMirrorOwner).(string), d.Get(pushMirrorRepo).(string), d.Id())

	return
}

func setPushMirrorResourceData(pushMirror *gitea.PushMirror, d *schema.ResourceData) (err error) {
	d.SetId(pushMirror.RemoteName)
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"sync_on_commit": {
				...
				Required:    false,
				Optional:    true,
				Description: "If the repository should be synced on every push",
			},
		},
